...
public final class PongGame extends BaseGame {
    ...
    @Override public void tick(){
        ...
        if(vx<0&&ballX>-154&&ballX<-140&&Math.abs(ballY-playerY)<23){ballX=-139;vx=Math.abs(vx)+5.0;vy+=(ballY-playerY)*4.0;}
        if(vx>0&&ballX<154&&ballX>140&&Math.abs(ballY-cpuY)<23){ballX=139;vx=-Math.abs(vx)-5.0;vy+=(ballY-cpuY)*3.2;}
        if(ballX<-185){cpuScore++;resetBall(1);}else if(ballX>185){playerScore++;resetBall(-1);}
    ...
    @Override public void render(DrawContext c,int mx,int my,float d){drawHeader(c,"PONG","W/S or Up/Down • You "+playerScore+" - "+cpuScore+" CPU");int cy=MinecraftClient.getInstance().getWindow().getScaledHeight()/2;int ox=cx();c.fill(ox-180,(int)(cy+playerY-22),ox-171,(int)(cy+playerY+22),0xFF55CC88);c.fill(ox+171,(int)(cy+cpuY-22),ox+180,(int)(cy+cpuY+22),0xFF55AADD);c.fill((int)(ox+ballX-5),(int)(cy+ballY-5),(int)(ox+ballX+5),(int)(cy+ballY+5),0xFFFFFFFF);}
}

...
public final class ArcadeCollection {
    ...
    public static final class FastClick extends BaseGame {
        ...
        private void move(){int h=MinecraftClient.getInstance().getWindow().getScaledHeight();int maxY=Math.max(90,h-30);ty=90+random.nextInt(maxY-90+1);tx=Math.max(24,Math.min(cx()-120+random.nextInt(241),MinecraftClient.getInstance().getWindow().getScaledWidth()-24));}

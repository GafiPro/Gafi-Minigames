...
public final class BattleshipGame extends BaseGame {
    ...
    @Override public void render(DrawContext c,int mx,int my,float d){drawHeader(c,"BATTLESHIP","Enemy grid • Blue: unknown • Red: hit • Gray: miss");int cell=Math.min(34,Math.max(20,heightSafe()/N)),ox=cx()+15-cell*N/2,oy=72;c.fill(ox-3,oy-3,ox+N*cell+3,oy+N*cell+3,0xFF173A5C);for(int y=0;y<N;y++)for(int x=0;x<N;x++){int color=shots[x][y]?(enemyShips[x][y]?0xFFE74C3C:0xFF777F86):0xFF2D6EA3;c.fill(ox+x*cell+1,oy+y*cell+1,ox+(x+1)*cell-1,oy+(y+1)*cell-1,color);}c.drawTextWithShadow(MinecraftClient.getInstance().textRenderer,Text.literal("Enemy ships: "+enemyShipsLeft+" • Your ships: "+shipsLeft),ox,oy+N*cell+10,0xFFFFFFFF);}

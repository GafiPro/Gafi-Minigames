...
public final class GamesScreen extends Screen {
    ...
    @Override public boolean mouseClicked(Click click,boolean doubled){if(click.button()!=GLFW.GLFW_MOUSE_BUTTON_1)return true;double mx=click.x(),my=click.y();int cx=width/2;String[]tabs={"ALL","ARCADE","PUZZLE","BOARD","ENDLESS","★ FAV"};for(int i=0;i<tabs.length;i++){int x=cx-255+i*85;if(inside(mx,my,x,35,78,18)){category=i==0||i==5?null:GameCatalog.Category.values()[i-1];favoritesOnly=i==5;page=0;return true;}}List<GameCatalog.Entry> list=filtered();int per=6,start=page*per,cw=210,ch=70,g=8,left=cx-cw-g/2;for(int j=0;j<per&&start+j<list.size();j++){int col=j%2,row=j/2,x=left+col*(cw+g),y=62+row*(ch+g);if(inside(mx,my,x,y,cw,ch)){GameCatalog.Entry e=list.get(start+j);MinigamesSettings.touchRecent(e.id());MinecraftClient.getInstance().setScreen(new GameScreen(this,GameFactory.create(e.id())));return true;}}return super.mouseClicked(click,doubled);}
    @Override public boolean keyPressed(KeyInput input){int key=input.key();if(key==GLFW.GLFW_KEY_ESCAPE){close();return true;}if(key==GLFW.GLFW_KEY_LEFT){page=Math.max(0,page-1);return true;}if(key==GLFW.GLFW_KEY_RIGHT){page++;return true;}if(key==GLFW.GLFW_KEY_F){List<GameCatalog.Entry> l=filtered();if(!l.isEmpty())MinigamesSettings.toggleFavorite(l.get(Math.min(page*6,l.size()-1)).id());return true;}if(key==GLFW.GLFW_KEY_ENTER){search=search.isEmpty()?"a":"";page=0;return true;}if((key>=GLFW.GLFW_KEY_A&&key<=GLFW.GLFW_KEY_Z)||key==GLFW.GLFW_KEY_SPACE||key==GLFW.GLFW_KEY_BACKSPACE){if(key==GLFW.GLFW_KEY_BACKSPACE&&!search.isEmpty())search=search.substring(0,search.length()-1);else if(key==GLFW.GLFW_KEY_SPACE)search+=" ";else search+=(char)('a'+key-GLFW.GLFW_KEY_A);page=0;return true;}return super.keyPressed(input);}

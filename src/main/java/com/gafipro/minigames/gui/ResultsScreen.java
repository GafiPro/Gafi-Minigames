...
public final class ResultsScreen extends Screen {
    ...
    @Override public void render(DrawContext c,int mx,int my,float d) {
        ...
        String headline=switch(state){case WON->"YOU WIN";case DRAW->"DRAW";case LOST->"GAME OVER";default->game.score()>0?"NICE RUN!":"RESULTS";};
        ...
        if(game instanceof BaseGame bg){
            var m=bg.metrics(); int line=y-15;
            ...
            if(m.accuracyPercent()<100){c.drawCenteredTextWithShadow(textRenderer,Text.literal("Accuracy: "+String.format("%.0f%%",(double)m.accuracyPercent())),x,line,0xFFBBBBBB);line+=18;}
        } else {

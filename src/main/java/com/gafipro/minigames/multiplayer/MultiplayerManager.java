...
public final class MultiplayerManager {
    ...
    private static void handle(Text message,GameProfile sender){String s=message.getString();int p=s.indexOf(PREFIX);if(p<0)return;int end=s.indexOf("]]",p);if(end<0)return;String encoded=s.substring(p+PREFIX.length(),end);String body;try{body=new String(Base64.getUrlDecoder().decode(encoded),StandardCharsets.UTF_8);}catch(IllegalArgumentException e){return;}String[]a=body.split("\\|",-1);if(a.length==0)return;MinecraftClient mc=MinecraftClient.getInstance();String senderName=profileName(sender);switch(a[0]){
        case "INVITE"->{if(a.length>=3&&senderName!=null){String id=a[1],game=a[2];Invite inv=new Invite(id,game,senderName,System.currentTimeMillis());invites.putIfAbsent(id,inv);if(mc.player!=null)mc.player.sendMessage(Text.literal("Gafi Minigames invite from "+senderName+" for "+game+". Open /games."),false);}}
        case "ACCEPT"->{if(a.length>=5&&senderName!=null){Match m=new Match(a[1],a[2],a[3],a[4],Objects.equals(playerName(),a[3]));if(senderName.equalsIgnoreCase(m.opponent)||senderName.equalsIgnoreCase(m.host)){matches.put(m.id,m);mc.execute(()->mc.setScreen(new MultiplayerGameScreen(null,m)));}}}
        case "DECLINE"->{if(a.length>=2)invites.remove(a[1]);}
        case "MOVE"->{if(a.length>=3&&senderName!=null){Match m=matches.get(a[1]);if(m!=null&&senderName.equalsIgnoreCase(m.opponent))m.remoteMove=a[2];}}
        case "REMATCH"->{if(a.length>=3&&senderName!=null){Match m=matches.get(a[1]);if(m!=null&&senderName.equalsIgnoreCase(m.opponent))m.rematchRemote="YES".equals(a[2]);}}

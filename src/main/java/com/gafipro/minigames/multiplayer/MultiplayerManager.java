...
public final class MultiplayerManager {
    ...
    private static void handle(Text message,GameProfile sender){String s=message.getString();int p=s.indexOf(PREFIX);if(p<0)return;int end=s.indexOf("]]",p);if(end<0)return;String enc=s.substring(p+PREFIX.length(),end),body;try{body=new String(Base64.getUrlDecoder().decode(enc),StandardCharsets.UTF_8);}catch(IllegalArgumentException e){return;}String[]a=body.split("\\|",-1);if(a.length==0)return;MinecraftClient mc=MinecraftClient.getInstance();String senderName=profileName(sender);purgeExpiredInvites();switch(a[0]){
        ...
        case "ACCEPT"->{if(a.length==5&&senderName!=null){OutgoingInvite pending=outgoingInvites.remove(a[1]);String self=playerName();if(pending!=null&&self!=null&&senderName.equalsIgnoreCase(pending.target())&&pending.gameId().equals(a[2])&&validGameId(a[2])){Match m=new Match(pending.matchId(),pending.gameId(),self,senderName,true);if(matches.putIfAbsent(m.id,m)==null)mc.execute(()->mc.setScreen(new MultiplayerGameScreen(null,m)));}}}

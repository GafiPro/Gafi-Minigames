...
public final class MultiplayerManager {
    ...
    private static void handle(Text message,GameProfile sender){
        ...
        try {
            byte[] decoded=Base64.getUrlDecoder().decode(enc);
            if(decoded.length>MAX_DECODED_PAYLOAD)return;
        ...
        case "ACCEPT"->{
            if(a.length==5&&senderName!=null){
                OutgoingInvite pending=outgoingInvites.get(a[1]); String self=playerName();
                if(pending!=null&&self!=null&&senderName.equalsIgnoreCase(pending.target())&&pending.gameId().equals(a[2])&&validGameId(a[2])&&self.equalsIgnoreCase(a[3])&&senderName.equalsIgnoreCase(a[4])){
                    outgoingInvites.remove(a[1],pending);
                    if(hasActiveMatchWith(self,senderName))return;
    ...
    private static boolean validMove(String game,String move){
        try{
            if(move==null||move.length()<3||move.length()>12||move.charAt(1)!=':')return false;
            int value=Integer.parseInt(move.substring(2));
            if(game.equals("tic_tac_toe")&&move.startsWith("T:"))return value>=0&&value<9;
            if(game.equals("connect_four")&&move.startsWith("C:"))return value>=0&&value<7;
            if(game.equals("rock_paper_scissors")&&move.startsWith("R:"))return value>=0&&value<3;
